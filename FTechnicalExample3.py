...
# 	Program should fail if number is negative
		self.assert_raises(out_of_range_error, to_roman, -1)
	
	def test_non_int(self):
# 	Program should fail with a non-integer input
		self.assert_raises(out_of_range_error, to_roman. 0.5)
...
# 	Program should fail with too many repeted numerals
		for s in ('MMMM', 'D', 'VV', 'LL', 'XXXX', 'CCCC', 'IIII'):
			self.assert_raises(bad_roman_numeral_error, from_roman, s)
...
# 	Application should only accept upper case
		for integer in range(1, 4000):
			...
			self.assert_raises(bad_roman_numeral_error, from_roman, numeral.lower())
